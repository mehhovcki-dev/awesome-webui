...
def get_invite_policy(config) -> dict:
    ...
    return {
        ...
        'creator_cooldown_seconds': max(0, _safe_int(getattr(config, 'INVITE_CREATOR_COOLDOWN_SECONDS', 0), 0)),
        'code_length': max(4, min(32, _safe_int(getattr(config, 'INVITE_CODE_LENGTH', 8), 8))),
        'code_ttl_seconds': max(0, _safe_int(getattr(config, 'INVITE_CODE_TTL_SECONDS', 604800), 604800)),
...
def can_user_create_invite_codes(
    ...
    latest_created_at = max(
        [_safe_int(invite.get('created_at'), 0) for invite in invite_codes if invite.get('created_by') == user.id]
        or [0]
    )
...
def create_invite_code(config, creator_id: str) -> dict:
    ...
    invite = {
        ...
        'uses_count': 0,
        'max_uses': (
            (None if policy['code_max_uses'] <= 0 else max(1, policy['code_max_uses']))
