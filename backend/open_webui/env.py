...
def _iter_data_dir_entries(path: Path):
    if not path.exists() or not path.is_dir():
        return []
    return [item for item in path.iterdir() if item.name not in {'.gitkeep', '.keep', '.DS_Store'}]
...
ENABLE_OPEN_WEBUI_DATA_MIGRATION = os.environ.get('ENABLE_OPEN_WEBUI_DATA_MIGRATION', 'True').lower() == 'true'
OPEN_WEBUI_DATA_MIGRATION_SOURCE = os.environ.get('OPEN_WEBUI_DATA_MIGRATION_SOURCE', '')
OPEN_WEBUI_DATA_MIGRATION_FORCE = os.environ.get('OPEN_WEBUI_DATA_MIGRATION_FORCE', 'False').lower() == 'true'
...
if ENABLE_OPEN_WEBUI_DATA_MIGRATION:
    ...
    can_migrate = OPEN_WEBUI_DATA_MIGRATION_FORCE or (not destination_has_data and not migration_marker.exists())

    if can_migrate:
        source_candidates = _find_open_webui_data_sources(OPEN_WEBUI_DATA_MIGRATION_SOURCE)
        for source in source_candidates:
            ...
            try:
                ...
                migration_marker.write_text(datetime.now(timezone.utc).isoformat(), encoding='utf-8')
                log.info('Open WebUI data migration completed successfully.')
                break
            except Exception as migration_error:
                log.warning(f'Failed to migrate data from {source}: {migration_error}')
...
WEBSOCKET_EVENT_CALLER_TIMEOUT = os.environ.get('WEBSOCKET_EVENT_CALLER_TIMEOUT', '60')
try:
...
WEBUI_GITHUB_REPOSITORY = os.environ.get('WEBUI_GITHUB_REPOSITORY', 'mehhovcki-dev/awesome-webui').strip()
if WEBUI_GITHUB_REPOSITORY == '':
    WEBUI_GITHUB_REPOSITORY = 'mehhovcki-dev/awesome-webui'

WEBUI_GITHUB_URL = os.environ.get('WEBUI_GITHUB_URL', f'https://github.com/{WEBUI_GITHUB_REPOSITORY}').rstrip('/')

WEBUI_GITHUB_RELEASES_URL = os.environ.get('WEBUI_GITHUB_RELEASES_URL', f'{WEBUI_GITHUB_URL}/releases').rstrip('/')
...
try:
    OTEL_METRICS_EXPORT_INTERVAL_MILLIS = int(os.environ.get('OTEL_METRICS_EXPORT_INTERVAL_MILLIS', '60000'))
except ValueError:
