...
if oauth_allowed_login_providers:
    try:
        ...
        oauth_allowed_login_providers = [
            provider.strip().lower() for provider in oauth_allowed_login_providers.split(',') if provider.strip()
        ]
    if not isinstance(oauth_allowed_login_providers, list):
        oauth_allowed_login_providers = []
    oauth_allowed_login_providers = [
        str(provider).strip().lower() for provider in oauth_allowed_login_providers if str(provider).strip()
    ]
else:
...
if oauth_allowed_signup_providers:
    try:
        ...
        oauth_allowed_signup_providers = [
            provider.strip().lower() for provider in oauth_allowed_signup_providers.split(',') if provider.strip()
        ]
    if not isinstance(oauth_allowed_signup_providers, list):
        oauth_allowed_signup_providers = []
    oauth_allowed_signup_providers = [
        str(provider).strip().lower() for provider in oauth_allowed_signup_providers if str(provider).strip()
    ]
else:
...
SLACK_ACCESS_TOKEN_URL = PersistentConfig(
    ...
    os.environ.get('SLACK_ACCESS_TOKEN_URL', 'https://slack.com/api/openid.connect.token'),
)

SLACK_AUTHORIZE_URL = PersistentConfig(
    'SLACK_AUTHORIZE_URL',
    'oauth.slack.authorize_url',
    os.environ.get('SLACK_AUTHORIZE_URL', 'https://slack.com/openid/connect/authorize'),
)
...
SLACK_USERINFO_ENDPOINT = PersistentConfig(
    ...
    os.environ.get('SLACK_USERINFO_ENDPOINT', 'https://slack.com/api/openid.connect.userInfo'),
)
